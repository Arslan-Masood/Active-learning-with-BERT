...
BASE_OUTPUT_DIR="/scratch/cs/pml/AI_drug/trained_model_pred/active_learning/ADME_ScaffoldSplitter"
...
echo "Debug: NUM_DATASETS = $NUM_DATASETS"
echo "Debug: NUM_FEATURES = $NUM_FEATURES"

# Calculate indices
TOTAL_COMBINATIONS=$((NUM_STRATEGIES * NUM_SEEDS))
DATASET_INDEX=$(( SLURM_ARRAY_TASK_ID / (TOTAL_COMBINATIONS * NUM_FEATURES) ))
REMAINDER=$(( SLURM_ARRAY_TASK_ID % (TOTAL_COMBINATIONS * NUM_FEATURES) ))
FEATURES_INDEX=$(( REMAINDER / TOTAL_COMBINATIONS ))
REMAINDER2=$(( REMAINDER % TOTAL_COMBINATIONS ))
SEED_INDEX=$(( REMAINDER2 / NUM_STRATEGIES ))
STRATEGY_INDEX=$(( REMAINDER2 % NUM_STRATEGIES ))
...
echo "Debug: DATASET_INDEX = $DATASET_INDEX"
...
TARGET_FILE="${BASE_DATA_DIR}/filtered_data/TDC_ADME/${DATASET}_filtered.csv"
BERT_FEATURES_FILE="${BASE_DATA_DIR}/MolBERT_features/MolBERT_${DATASET}.csv"
ECFP_FEATURES_FILE="${BASE_DATA_DIR}/MF/MF_r2_1024_${DATASET}.csv"

POS_WEIGHTS_FILE="${BASE_DATA_DIR}/filtered_data/TDC_ADME/${DATASET}_pos_ratio.csv"
METADATA_DIR="${BASE_OUTPUT_DIR}/${DATASET}/${FEATURES_TYPE}"
...
# Update strategy, seed, project name, and dataset
sed -i "s/\"sampling_strategy\": \"[^\"]*\"/\"sampling_strategy\": \"$STRATEGY\"/" "$TMP_CONFIG"
sed -i "s/\"seed\": [0-9]*/\"seed\": $SEED/" "$TMP_CONFIG"
